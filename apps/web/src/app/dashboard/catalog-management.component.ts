...
export class CatalogManagementComponent {
  ...
  businessId = 0; published = false; categories: Category[] = []; products: Product[] = []; newCategory = ''; editingCategory?: Category; editingProduct?: Product; image?: File; ingredientInput = '';
  product: ProductInput = { category_id: 0, name: '', description: '', price: null, is_active: true, ingredients: [] };
  modal: 'categories' | 'product' | 'confirm' | undefined; pendingDelete?: { type: 'category' | 'product'; name: string; id: number };

  constructor() { this.businessApi.getMine().subscribe(({ business }) => { this.businessId = business.id; this.published = !!business.is_published; this.load(); }); }
  ...
  openNewProduct(): void { this.editingProduct = undefined; this.image = undefined; this.ingredientInput = ''; this.product = { category_id: this.categories[0]?.id ?? 0, name: '', description: '', price: null, is_active: true, ingredients: [] }; this.modal = 'product'; }
  ...
  saveProduct(): void { if (!this.product.name.trim() || !this.product.category_id) return; const request = this.editingProduct ? this.productsApi.update(this.businessId, this.editingProduct.id, this.product, this.image) : this.productsApi.create(this.businessId, this.product, this.image); request.subscribe(() => { this.resetProduct(); this.load(); }); }
  editProduct(item: Product): void { this.editingProduct = item; this.image = undefined; this.ingredientInput = ''; this.product = { category_id: item.category_id, name: item.name, description: item.description ?? '', price: item.price == null ? null : Number(item.price), is_active: item.is_active, ingredients: [...(item.ingredients ?? [])] }; this.modal = 'product'; }
  resetProduct(): void { this.editingProduct = undefined; this.image = undefined; this.ingredientInput = ''; this.product = { category_id: this.categories[0]?.id ?? 0, name: '', description: '', price: null, is_active: true, ingredients: [] }; this.modal = undefined; }
  closeModal(): void { this.modal = undefined; this.editingCategory = undefined; this.pendingDelete = undefined; }
  togglePublish(): void { this.businessApi.publish(this.businessId, !this.published).subscribe(() => this.published = !this.published); }
