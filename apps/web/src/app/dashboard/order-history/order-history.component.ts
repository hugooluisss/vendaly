...
export class OrderHistoryComponent {
  ...
  constructor() { this.businessApi.getMine().subscribe({ next: ({ business }) => { this.businessId = business.id; this.businessApi.orderStatuses(this.businessId).subscribe({ next: statuses => this.statuses = statuses, error: () => this.error = 'No se pudieron cargar los estados de los pedidos.' }); this.load(); }, error: () => this.error = 'No se pudo cargar el negocio.' }); }
